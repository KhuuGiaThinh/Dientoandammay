...
from database import FoodItem, Order, OrderItem, configure_database, db, init_db_with_retry
...
def fetch_food(food_id):
    # Query database directly instead of calling HTTP endpoint
...
def seed_data():
    # Seed FoodItems
    if not db.session.execute(db.select(FoodItem.id)).first():
    ...
    # Seed Orders
    if not db.session.execute(db.select(Order.id)).first():
